...
class Day03PerfectlySpericalHousesInAVacuum(filename: String) extends DailyProblem[Int, Int] {
  ...
  override val part1Answer: Int = moves.toCharArray.foldLeft((Point(0, 0), Set(Point(0, 0))))((acc, c) => {
    val newP: Point = calculateMove(acc._1, c)
    (newP, acc._2 + newP)
  })._2.size

  override val part2Answer: Int = moves.toCharArray.foldLeft((true, Point(0, 0), Point(0, 0), Set(Point(0, 0))))((acc, c) => {
    if (acc._1) {
